...
model = load_model('chestmodel_mobilenet.keras')

# Define class labels
class_labels = ['adenocarcinoma', 'large_cell_carcinoma', 'normal', 'squamous_cell_carcinoma']

# Function to preprocess the image for prediction (based on MobileNet requirements)
def preprocess_for_prediction(image):
    img = cv2.resize(image, (224, 224))  # Resize to the input size of MobileNet (224x224)
    img = cv2.cvtColor(img, cv2.COLOR_GRAY2RGB)  # Convert grayscale to RGB
    img = img.astype('float32')  # Convert image to float32
    img = preprocess_input(img)  # MobileNet-specific preprocessing
